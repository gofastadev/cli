...
// {{.Name}}Controller exposes the REST surface for the {{.Name}}
// resource. Generated by ` + "`gofasta g controller`" + ` — every
// handler delegates the actual work to the service layer.
type {{.Name}}Controller struct {
	{{.Name}}Service svcInterfaces.{{.Name}}ServiceInterface
}

// New{{.Name}}ControllerInstance constructs a {{.Name}}Controller bound
// to the supplied service. Wire calls this at app startup.
...
// List handles GET /{{.PluralLower}} — returns the paginated set with
...
// GetByID handles GET /{{.PluralLower}}/{id} — returns a single record.
{{- if .IncludeSwagger}}
//
...
// Create handles POST /{{.PluralLower}} — creates a new record.
{{- if .IncludeSwagger}}
//
...
// Update handles PUT /{{.PluralLower}}/{id} — applies a partial update.
{{- if .IncludeSwagger}}
//
...
// Archive handles DELETE /{{.PluralLower}}/{id} — soft-deletes a record.
{{- if .IncludeSwagger}}
//
