...
func writeMockForTarget(t interfaceTarget, module string, opts GenMockOpts) error {
	...
	body, err := renderMock(d)
	if err != nil {
		return clierr.Wrap(clierr.CodeMockGenFailed, err, "rendering mock for "+t.Name)
	}
...
func renderMock(d MockData) ([]byte, error) {
	var b bytes.Buffer
	...
	if err != nil {
		// Return unformatted on format error so the user sees the actual
		// generator output and can fix the underlying issue.
		return b.Bytes(), nil
	}
